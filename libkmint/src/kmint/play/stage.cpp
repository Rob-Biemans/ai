...
stage::stage(math::size size) : size_{size},
                                overlay_{std::make_unique<stage_overlay>(*this)} {}
  ...
  for(actor &a: *this) {
    if (!a.incorporeal()) {
      ...
      std::copy_if(candidates.begin(),
                   candidates.end(),
                   a.collision_adder(),
                   [](actor const *candidate) {
                     return !candidate->incorporeal();
                   });
      candidates.clear();
    }
    if (a.perceptive()) {
...
const ui::drawable &stage::overlay() const {
  return *overlay_;
}
